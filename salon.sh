...
GET_SERVICES_ID() {
  ...
  do
    ID_USER=$(echo $SERVICE_ID | sed 's/ //g')
    NAME_USER=$(echo $SERVICE | sed 's/ //g')
    echo "$ID_USER) $SERVICE"
  done
...
NEXT() {
  ...
  NAME_USER=$($PSQL "SELECT name FROM customers WHERE phone='$CUSTOMER_PHONE'")
  CUSTOMER_NAME=$(echo $NAME_USER | sed 's/ //g')
  if [[ -z $NAME ]]
  then
    echo -e "\nI don't have a record for that phone number, what's your name?"
    read CUSTOMER_NAME
    NAME_USER=$(echo $NAME_USER | sed 's/ //g')
    SAVED_TO_TABLE_CUSTOMERS=$($PSQL "INSERT INTO customers(name,phone) VALUES('$NAME_USER','$CUSTOMER_PHONE')")
  fi
